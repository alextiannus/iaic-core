...
try{
 ...
 const save=defineCapability({name:'notes.save',description:'Save the requested writing preference.',input:{type:'object',properties:{content:{type:'string',const:content}},required:['content'],additionalProperties:false},output,effect:'write',retry:'never-replay',authorize:authorized,revalidate,implementation:{kind:'function',execute:async input=>{const row=await memories.remember(scope,{key:'writing-style',kind:'preference',content:input.content,source:{kind:'user-statement',reference:actor.subjectId},expectedRevision:0});return {content:row.content,revision:row.revision};}}});
 ...
 let step=0;const actions=[{type:'call',name:'notes.save',input:{content}},{type:'call',name:'notes.read',input:{}},{type:'finish',result:{remembered:true}}];
 const live=Boolean(process.env.DEMO_MODEL_API_KEY);
 const model=live?createModelProvider({apiKey:process.env.DEMO_MODEL_API_KEY,model:process.env.DEMO_MODEL,provider:process.env.DEMO_PROVIDER||'openai',baseUrl:process.env.DEMO_BASE_URL||''}):{name:'deterministic-reference',next:async()=>actions[step++]};
 ...
 console.log(JSON.stringify({application:'core-notes',modelMode:live?'provider':'deterministic',task:finished.status,persisted:true,memoryExportVerified:true,memoryImportVerified:true,memoryRelearningVerified:true,memoryDisputeVerified:true,erpUsed:false}));
