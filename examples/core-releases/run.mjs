...
try{
 ...
 const observation=new ReleaseObservation({store:observations,releases:manager,sourceScope:'example',authorize:a=>a.subjectId==='platform',now:()=>clock,policies:{production:{revision:'strict-fixture-budget',fallbackId:'fixture-v1',minSamples:1,maxErrorRate:0,maxMeanLatencyMs:0,windowMs:60000,maxAssessmentAgeMs:60000}},resolveSource:sourceId=>({confirmed:true,sourceId,sourceScope:'example',record:{releaseId:selected.releaseId,manifestDigest:selected.manifestDigest,observedAt:new Date(clock.getTime()-1).toISOString(),success:executed.status==='succeeded',durationMs:executed.durationMs,toolErrors:0,providerTokens:'0',platformUnits:'0'}})});
 const monitor=new CapabilityDispatcher({capabilities:createObservationCapabilities({observation})});await monitor.invoke('observation.record',{sourceId:'container-result'},{actor,callId:'observe'});
 ...
 await monitor.invoke('observation.protect',{assessmentId:assessment.id,expectedRevision:2},{actor,callId:'protect'});await assert.rejects(codeDispatcher.invoke('code.run',request,{actor,callId:randomUUID()}));await assert.rejects(manager.check(actor,selected),{statusCode:409});
 ...
 console.log(JSON.stringify({example:'core-releases',status:'passed',boundEvaluationEvidence:true,canaryStopped:true,rollbackSelection:true,materializedResources:true,containerExecution:true,stoppedCodeDenied:true,observedAtomicRollback:true,fallbackExecuted:true,productionDeployed:false}));
