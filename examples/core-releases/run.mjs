...
import {Pool} from 'pg';
import {EvaluationRunner,FileEvaluationStore,ReleaseManager,PostgresReleaseStore,createReleaseCapabilities,CapabilityDispatcher} from '@immedi/iaic-core';
...
try{
 ...
 for(const run of runs)await manager.register(actor,{manifest:{id:run.revision,implementationRevision:run.revision,versions:{prompt:'fixture1',model:'fixture1',skills:'none1',tools:'fixture1',knowledge:'none1',harness:'fixture1'}},evaluations:{capability:run.id,regression:run.id}});
 ...
 console.log(JSON.stringify({example:'core-releases',status:'passed',boundEvaluationEvidence:true,canaryStopped:true,rollbackSelection:true,productionDeployed:false}));
