...
try{
 ...
 const candidateResources=await resources.read(agentActor,selected);
 const candidateAgent=await buildAgent(selected,candidateResources.files.find(file=>file.path==='prompts/main.txt').bytes.toString('utf8'));
 ...
 const fallbackResources=await resources.read(agentActor,fallback),fallbackAgent=await buildAgent(fallback,fallbackResources.files.find(file=>file.path==='prompts/main.txt').bytes.toString('utf8'));
 ...
 console.log(JSON.stringify({example:'core-releases',status:'passed',boundEvaluationEvidence:true,canaryStopped:true,rollbackSelection:true,materializedResources:true,inMemoryAgentResources:true,containerExecution:true,stoppedCodeDenied:true,observedAtomicRollback:true,trustedCollectionCycle:true,fallbackExecuted:true,originalProtectionReceipt:true,persistentCycle:true,stoppedAgentBlocked:true,fallbackAgentExecuted:true,productionDeployed:false}));
