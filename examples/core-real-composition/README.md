...
The current harness supplies Memory listing. Grader v2 recognizes the requested preference whether read by exact key or observed through the list result, since both expose the same source content. This corrects the fixture scope and grading assumption; it was a changed evaluation configuration awaiting a new actual-model run at that revision. Future results must remain separate from v1. The multiple-call incompatibility remains a Core model/Runtime capability gap; the harness does not drop calls, silently execute batches, or relax outcome checks to hide it.
