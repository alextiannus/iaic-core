...
export const fixtureFactory=()=>({next:async({messages})=>{
 ...
 else if(calls.length===5){const ready=JSON.parse(calls[4].result.text).filter(r=>r.state==='ready'&&(process.env.IAIC_CONTINUITY_CLARIFY!=='1'||r.id!=='C')),project=c.session.events.find(e=>e.kind==='user_message').data.text.match(/Project-[a-f0-9]+/)[0];action={type:'call',name:'my_write_workspace',input:{path:'result.json',content:JSON.stringify({project,selectedIds:process.env.IAIC_CONTINUITY_CLARIFY==='1'?ready.map(r=>r.id).sort():ready.map(r=>r.id).sort().reverse(),totalUnits:ready.reduce((n,r)=>n+r.units,0)}),expectedRevision:0}};}
