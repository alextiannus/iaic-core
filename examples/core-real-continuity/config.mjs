...
const planning=process.env.IAIC_CONTINUITY_PLANNING==='1';
if(planning&&!memoryUpdate)throw new Error('Planning evaluation requires the explicit once-only memory scenario');
export const tools=['assistant.skills.list','assistant.skills.read','my_search_knowledge','my_read_knowledge','my_read_assistant_memory','my_list_assistant_memories','my_write_workspace','my_read_workspace','my_read_assistant_session',...(memoryUpdate?['my_remember_assistant_memory']:[]),...(planning?['tasks.plan.read','tasks.plan.update']:[])];
export const options={skillRoot:fileURLToPath(new URL('./skills/',import.meta.url)),version:process.env.IAIC_SOURCE_REVISION,
 ...(planning?{enablePlans:true,toolCallLimits:{my_remember_assistant_memory:1}}:{}),
