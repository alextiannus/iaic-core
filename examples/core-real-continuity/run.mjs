...
const clarificationMode=process.env.IAIC_CONTINUITY_CLARIFY==='1';
const runId=randomUUID(),project='Project-'+runId.slice(0,8),records=[{id:'A',state:'ready',units:7},{id:'B',state:'draft',units:90},{id:'C',state:'ready',units:11},...(clarificationMode?[{id:'D',state:'ready',units:4}]:[])],expected={project,selectedIds:clarificationMode?['A','D']:['C','A'],totalUnits:clarificationMode?11:18};
...
const goal='Use the installed source-summary method, my style preference, the working guide and this session to prepare result.json. Read back the saved artifact, then finish. This is a working draft only.'+(clarificationMode?' Before preparing the artifact, ask me which record IDs to exclude and whether to override the saved ID sort order. These choices have not been provided. Do not write the artifact until I clarify.':'');
const frozen={runId,source:options.version,model:process.env.IAIC_MODEL,provider:process.env.IAIC_PROVIDER||'chat-completions',limits:options.runtimeLimits,toolScope:tools,graderRevision:clarificationMode?'continuity-clarification-v1':'continuity-v2',project,records,expected,goal,...(clarificationMode?{clarification}:{}),skillDigest:evidenceDigest(await fs.readFile(new URL('./skills/source-summary/SKILL.md',import.meta.url),'utf8')),independentHoldout:false};
