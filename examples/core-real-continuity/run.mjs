...
const clarificationMode=process.env.IAIC_CONTINUITY_CLARIFY==='1',memoryUpdate=process.env.IAIC_CONTINUITY_MEMORY_UPDATE==='1',planning=process.env.IAIC_CONTINUITY_PLANNING==='1';
...
const goal=(memoryUpdate?'First update my existing style memory exactly once to: '+updatedPreference+' This is an explicitly authorized lasting preference change; preserve the key style and use its current revision. Then ':'')+'Use the installed source-summary method, my style preference, the working guide and this session to prepare result.json. Read back the saved artifact, then finish. This is a working draft only.'+(clarificationMode?' Before preparing the artifact, ask me which record IDs to exclude and whether to override the saved ID sort order. These choices have not been provided. Do not write the artifact until I clarify.':'')+(planning?' Maintain a brief persistent Task plan while working, using the plan tools. Plan status is a working claim; verify the actual artifact before finishing.':'');
const frozen={runId,source:options.version,model:process.env.IAIC_MODEL,provider:process.env.IAIC_PROVIDER||'chat-completions',limits:options.runtimeLimits,toolScope:tools,...(planning?{planning:true,toolCallLimits:options.toolCallLimits}:{}),graderRevision:planning?'continuity-memory-plan-v1':memoryUpdate?'continuity-memory-update-v1':clarificationMode?'continuity-clarification-v1':'continuity-v2',project,records,expected,goal,...(memoryUpdate?{updatedPreference,expectedMemoryRevision:2}:{}),...(clarificationMode?{clarification}:{}),skillDigest:evidenceDigest(await fs.readFile(new URL('./skills/source-summary/SKILL.md',import.meta.url),'utf8')),independentHoldout:false};
...
try{
 ...
 const savedMemory=memoryUpdate?await app.memory.read(actor,{key:'style'}):null,savedPlan=planning?await app.plans.read(actor,{id:task.id}):null;
 if(memoryUpdate)checks.memory_updated_once=savedMemory.content===updatedPreference&&savedMemory.revision===2&&successful.filter(c=>c.capability==='my_remember_assistant_memory').length===1;
 if(planning)checks.plan_saved=savedPlan.taskId===task.id&&savedPlan.revision>0&&savedPlan.steps.length>0&&successful.some(c=>c.capability==='tasks.plan.update');
