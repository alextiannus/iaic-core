...
try{
 ...
 const capabilities=tools.map(tool=>defineCapability({name:tool.name,description:tool.description,input:tool.inputSchema,output:{type:'object'},effect:tool.name==='my_write_workspace'?'write':'read',...(tool.name==='my_write_workspace'?{retry:'never-replay'}:{}),authorize,...(tool.projectHistoryInput?{projectHistoryInput:tool.projectHistoryInput}:{}),
