...
try {
 ...
 let boundedCalls=0;
 const profiles=new ModelProfiles({profiles:[{id:'bounded',model:'fixture',provider:'chat-completions',baseUrl:'https://fixture.example/v1',credentialRef:'fixture',invocation:{maxCompletionTokens:8192}}],resolveSecret:async()=> 'fixture',factory:config=>createModelProvider({...config,fetchImpl:async(_url,options)=>{
  const wire=JSON.parse(options.body);assert.equal(wire.max_completion_tokens,8192);assert.equal(Object.hasOwn(wire,'max_tokens'),false);boundedCalls++;
 ...
 console.log(JSON.stringify({example:'core-model-routing',status:'passed',persistentActualModel:true,serviceReconstruction:true,newTaskUsesCurrentAvailability:true,existingMetering:true,totalCompletionBudget:true,truncatedUsageSettled:true,modelMode:'deterministic',processKill:false}));
