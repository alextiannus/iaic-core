...
iaic init DIRECTORY --core-package ARCHIVE
...
export async function runCli(args,{env=process.env}={}){
 ...
 const allowed={list:['--url'],call:['--url','--input','--request-key'],init:['--core-package'],migrate:['--config']}[command];
 ...
 if(command==='init'){if(!options['--core-package'])throw new Error('--core-package is required');return scaffoldCapabilityApp({directory:positionals[0],corePackage:options['--core-package']});}
 if(command==='migrate'){
