...
export async function runCli(args,{env=process.env}={}){
 ...
 const allowed={list:['--url'],call:['--url','--input','--request-key'],init:['--core-package','--template'],migrate:['--config'],deploy:['--config','--request-key'],'deployment-status':['--config','--request-key'],'deployment-stop':['--config','--request-key']}[command];
