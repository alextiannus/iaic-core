...
export const delegationProposalSchema={type:'object',properties:{goal:{type:'string',minLength:1,maxLength:4000},successCriteria:{type:'string',minLength:1,maxLength:2000},tools:{type:'array',items:{type:'string'},maxItems:100,uniqueItems:true},artifacts:{type:'array',items:reference,maxItems:20},requiredArtifacts:{type:'array',items:{type:'string',minLength:1,maxLength:300},maxItems:20,uniqueItems:true}},required:['goal','successCriteria','tools'],additionalProperties:false};
...
export class Delegations {
 ...
 schema(task){return this.policy(task.input)&&!task.handoff&&!task.delegation?{...delegationProposalSchema,properties:{...delegationProposalSchema.properties,tools:{...delegationProposalSchema.properties.tools,items:{type:'string',...(task.input.allowedTools.length?{enum:task.input.allowedTools}:{})},maxItems:task.input.allowedTools.length}}}:undefined;}
