...
A Runtime integration reproduces and fixes completed-but-invalid actions retaining capacity: the first completed response selects an undeclared action, the Runtime records feedback, and a second completed response finishes successfully. Both measured model responses are charged once; capacity is released between them. providerCompleted denotes supplier completion, not a valid model action, successful Task or zero usage. Unknown transport errors still retain their original slot.
