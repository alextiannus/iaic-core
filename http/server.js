...
export function createCapabilityHttpHandler({dispatcher, resolveAccess, basePath = '/capabilities', surface = 'model'}) {
  ...
  return async request => {
    ...
    try {
      ...
      if (listing) return json({capabilities: capabilities.filter(c=>capabilityVisible(c,surface)).map(c => ({
      ...
      started = true;
      const result = await dispatcher.invoke(name, body.input, {actor: access.actor, callId: requestKey ?? null, signal: request.signal, allowedCapabilities: access.capabilities,surface});
