...
export function createSupportCapabilities({support,prefix='support'}){
 const schemas={report:object({requestKey:key,summary:{type:'string',minLength:1,maxLength:2000},taskId:key,requestId:key,releaseId:key,errorCode:key},['requestKey','summary']),get:object({id},['id']),list:object({limit:{type:'integer',minimum:1,maximum:100}},[]),queue:object({limit:{type:'integer',minimum:1,maximum:100}},[]),update:object({id,expectedRevision:{type:'integer',minimum:1},state:{enum:['triaged','fixing','verifying','resolved','closed','reopened']},message:{type:'string',minLength:1,maxLength:2000},evidenceId:key},['id','expectedRevision','state','message']),notify:object({id,revision:{type:'integer',minimum:1}},['id','revision'])};
