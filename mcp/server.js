...
export function createCapabilityMcpServer({dispatcher,resolveAccess,serverInfo={name:'iaic-capabilities',version:'0.1.0'}}){
 ...
 server.setRequestHandler(ListToolsRequestSchema,async(request,extra)=>{
  ...
  return {tools:access.capabilities.map(cap=>{
 ...
 server.setRequestHandler(CallToolRequestSchema,async(request,extra)=>{
  ...
  try{
   ...
   started=true;
   const result=await dispatcher.invoke(cap.name,envelope.input,{actor:access.actor,callId:key??null,signal:extra.signal,allowedCapabilities:access.capabilities.map(cap=>cap.name)});
