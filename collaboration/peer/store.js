...
export class PostgresPeerStore{
 ...
 async problems(limit=50){return (await this.pool.query(`SELECT n.id AS "deliveryId",n.scope_id AS "channelId",n.request_key AS "requestKey",n.message->>'id' AS "messageId",n.state,n.created_at AS "createdAt" FROM iaic_notifications n JOIN iaic_peer_records c ON c.namespace=$2 AND c.kind='channel' AND c.id=n.scope_id WHERE n.namespace=$1 AND (n.state='unknown' OR (n.state='failed' AND (SELECT count(*) FROM iaic_notification_attempts a WHERE a.notification_id=n.id)>=(c.data->>'maxAttempts')::integer)) AND NOT EXISTS(SELECT 1 FROM iaic_peer_records r WHERE r.namespace=$2 AND r.kind='delivery-escalation' AND r.id=n.id::text) ORDER BY n.created_at,n.id LIMIT $3`,[this.deliveryNamespace,this.namespace,limit])).rows;}
