...
export class PeerDeliveryRecovery{
 ...
 async tick(options={}){
  ...
  for(const job of problems){const context=await this.peer.store.transaction(async tx=>{const c=await tx.get('channel',job.channelId),history=await tx.outbox.history(c.id,job.requestKey);if(job.state==='failed'&&history.length<c.maxAttempts)return null;return {channel:c,job,history};});if(!context)continue;
