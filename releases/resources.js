...
export class ReleaseResources{
 ...
 async read(actor,reference){
  const binding=Object.freeze({releaseId:reference?.releaseId,manifestDigest:reference?.manifestDigest});
  const manifest=await this.releases.check(actor,binding),resources=structuredClone(manifest.resources);
  ...
  for(const item of resources){
   ...
   files.push({path:item.path,sha256:item.sha256,byteLength:item.byteLength,bytes});
  }
  await this.releases.check(actor,binding);
  return {...binding,files};
 }
 async materialize(actor,reference,{parentDirectory}){
  const loaded=await this.read(actor,reference);
  const binding=Object.freeze({releaseId:loaded.releaseId,manifestDigest:loaded.manifestDigest});
  const directory=await fs.mkdtemp(path.join(path.resolve(parentDirectory),'iaic-release-'));
  try{
   for(const item of loaded.files){
    const file=path.join(directory,item.path);await fs.mkdir(path.dirname(file),{recursive:true});await fs.writeFile(file,item.bytes,{flag:'wx',mode:0o600});
   }
   await this.releases.check(actor,binding);
   return {directory,...binding,files:loaded.files.map(({path,sha256,byteLength})=>({path,sha256,byteLength}))};
