...
Latest user clarification (2026-09-14): prioritize Platform AI capabilities; provide User Assistant foundations and a generic demo. Both can be application-provided or external third-party Agents. Business AI is application-resident, uses system model configuration and system/server capabilities, and is not a required built-in Core product. Core supplies cross-application Business AI capabilities only where needed. This supersedes older role-delivery assumptions; see CORE_REQUIREMENTS.md.
...
## Foundation coverage and remaining integration work

The design's complete capability map remains the target; headless design does not remove foundational services from that scope. Current modules are usable building blocks, not completion of the entire map. The overview below is not a strict next-task queue. Later dated entries retain their historical evidence; use the current source and ACCEPTANCE.md before treating an old limitation as missing implementation.
...
5. Developer platform: an installed CLI now lists/calls shared HTTP capabilities, scaffolds a local Capability application from an explicit Core archive, and runs versioned PostgreSQL migrations with history checks and transactional batches. A persistent Agent starter now composes identity, Runtime, scoped resources, Session, models and allowance, with a separate-process queued-work test. A default Docker deployment adapter and CLI now deploy an immutable application image, inspect its live health/endpoint and stop it while retaining the original receipt. Real Docker and separate CLI processes verify stable instance reuse and a lost start acknowledgement. The Agent template also wires optional one-time scheduling, event-driven queueing, working plans, Mandates, model routing and evaluated-release binding into existing modules. See developer/templates/agent/app.mjs and its README. LocalSimulation and scripted-model tooling exist separately. Image building, cloud provider adapters, durable desired-state reconciliation, traffic activation/rollback and template handoff wiring remain incomplete; broader simulation support is not claimed.
...
## Note 43 design and delivery alignment

The source-confirmed scheduling, event and Mandate template compositions are now
reflected in the foundation overview; they should not be rebuilt from old status
text. CORE_REQUIREMENTS.md now distinguishes User, Business and Platform
responsibilities without forcing Runtime placement or three role classes.
Application report quality remains separate from Core foundation delivery;
