...
- Persistent Task collaboration remains bounded and same-owner. A separate cross-principal function-capability grant now intersects issuer/delegate authority, preserves payer attribution and atomically bounds call admissions; it does not yet bind Agent Tasks. A separate shared allowance-budget primitive now bounds multiple executors under one payer ledger, retains unknown holds and supports explicitly platform-absorbed overflow. Binding these budgets to durable cross-principal Tasks remains incomplete. General cross-principal delegation, full A2A conformance, every MCP protocol feature and arbitrary remote-tool installation are not claimed.
