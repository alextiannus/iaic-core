...
export class ContextAssembler {
  constructor({skillRoot,maxBytes=120000,memoryProvider=null,skillCatalog=null,sessionProvider=null,handoffProvider=null}) {this.skillRoot=skillRoot;this.maxBytes=maxBytes;this.memoryProvider=memoryProvider;this.skillCatalog=skillCatalog;this.sessionProvider=sessionProvider;this.handoffProvider=handoffProvider;}
  async assemble({task,capability,history,actor,dispatcher}) {
    const skills=[];
  ...
  async revalidateHistory({history,actor,dispatcher}) {
    ...
    for(const call of history.calls) {
      ...
      if(call.status==='succeeded') {
        ...
        result=await target.revalidate(call.input,call.result,{actor,callId:call.id});
      }
