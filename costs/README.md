...
These numbers illustrate the schema and are not current prices for any provider. Rates are nonnegative integer currency minor units per one million provider tokens. The host must supply current applicable rates, the currency's correct scale, provider account attribution and a matching model identity. Currency codes are format-checked, not looked up; Core performs no exchange-rate conversion or provider-price scraping. For SYSTEM_MANAGED the bearer is platform; for BYOK it is user. An account reference is an opaque attribution identifier, never an API key.
