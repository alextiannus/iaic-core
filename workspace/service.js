...
export class AssistantWorkspace{
 ...
 async list(actor,input={}){
  const page=await this.store.list(await this.resolveScope(actor),input);if(!this.lineage)return page;
 ...
 async read(actor,input){const artifact=await this.store.read(await this.resolveScope(actor),input);if(this.lineage)await this.lineage.check({actor,artifact});return artifact;}
 async write(actor,input,context={}){const scope=await this.resolveScope(actor);let source=await this.sourceFor(actor);if(this.lineage)source=await this.lineage.prepare({actor,input,source,context});return this.store.write(scope,{path:input.path,content:input.content,mediaType:input.mediaType,expectedRevision:input.expectedRevision??0,source});}
 async purgeInvalid(actor,input={}){
  ...
  const scope=await this.resolveScope(actor),page=await this.store.list(scope,input),removed=[];
